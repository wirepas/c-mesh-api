...
{
    // Use value initialization for WpcFrame so that the underlying payload
    // array is initialized with zeroes. This makes it easier to compare two
    // arrays for assertions in the tests.
    WpcFrame decoded {};
    size_t read_index;
    size_t write_index;
    bool in_escape;
...
void SerialStub::EncodeAndAddToOutQueue(const uint8_t byte)
{
    ...
    {
        out_queue.push_back(Slip::ESC);
        out_queue.push_back(Slip::ESC_ESC);
    }
    else
    {
