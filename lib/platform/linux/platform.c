...
static void * poll_for_indication(void * unused)
{
     ...
     while (m_polling_thread_state_request != POLLING_THREAD_STOP)
    {
        ...
        if(m_polling_thread_state_request == POLLING_THREAD_STOP_REQUESTED)
        {
            ...
            LOGI("Waiting for reassembly queue to be empty\n");
        }
        ...
        max_num_indication = (m_polling_thread_state_request == POLLING_THREAD_STOP_REQUESTED)
                                 ? 1
                                 : MIN(MAX_NUMBER_INDICATION, free_buffer_room);              
