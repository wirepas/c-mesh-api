...
#define FHT_INVALID_SLOT     ((size_t) -1)
...
static size_t probe(const fht_t * t, uint32_t node_id, int * found)
{
    ...
    *found = 0;
    ...
    for (size_t i = 0; i < t->capacity; i++)
    {
        ...
        {
            return (tomb != FHT_INVALID_SLOT) ? tomb : pos;
        }
        if (s->state == SLOT_TOMBSTONE)
        {
            if (tomb == FHT_INVALID_SLOT)
                tomb = pos;
            continue;
        }
...
static int resize(fht_t * t, size_t new_cap)
{
    ...
    size_t       old_tombstones = t->tombstones;
    ...
    for (size_t i = 0; i < old_n; i++)
    {
        ...
        if (pos == FHT_INVALID_SLOT || found)
        {
            Platform_free(fresh, new_cap * sizeof(*fresh));
            t->slots      = old;
            t->capacity   = old_n;
            t->count      = old_count;
            t->tombstones = old_tombstones;
            return -1;
        }
        t->slots[pos] = old[i];
