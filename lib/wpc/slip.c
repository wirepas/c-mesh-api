 ...
 */
static uint16_t crc_fromBuffer(uint8_t * buf,
                               uint32_t len)
{
    uint16_t crc = 0xffff;
    uint8_t index;
 ...
 */
static uint32_t slip_decode_buffer(uint8_t* buffer,
                                   uint32_t len)
{
    uint8_t write = 0;
    uint32_t read;
 ...
 * \return  length of the escaped frame or -1 if the escaped frame cannot feet the provided buffer
 */
static uint32_t slip_encode_buffer(uint8_t * buffer,
                                   uint32_t len,
                                   uint8_t * buffer_escaped,
                                   uint32_t len_escaped)
{
    uint8_t write = 0;
    uint32_t read;
    for (read = 0; read < len; read++) {
