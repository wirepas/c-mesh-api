...
{
    return WPC_Int_initialize(port_name, bitrate) == 0 ?
        APP_RES_OK :
...
{
    uint8_t att = role;
    int res = csap_attribute_write_request(C_NODE_ROLE_ID,
                                 1,
                                 &att);
...
app_res_e WPC_get_node_address(app_addr_t * addr_p)
{
    app_res_e ret;
    uint8_t att[4];
    int res = csap_attribute_read_request(C_NODE_ADDRESS_ID,
                                4,
                                att);
...
app_res_e WPC_set_node_address(app_addr_t add)
{
    ...
    int res = csap_attribute_write_request(C_NODE_ADDRESS_ID,
                                 4,
                                 att);
...
app_res_e WPC_get_network_address(net_addr_t * addr_p)
{
    ...
    att[3] = 00;
    int res = csap_attribute_read_request(C_NETWORK_ADDRESS_ID,
                                3,
                                att);
...
app_res_e WPC_set_network_address(net_addr_t add)
{
    uint8_t att[4];
    uint32_encode_le(add, att);
    int res = csap_attribute_write_request(C_NETWORK_ADDRESS_ID,
                                           3,
                                           att);
...
{
    int res = csap_attribute_read_request(C_NETWORK_CHANNEL_ID,
                                          1,
                                          channel_p);
...
app_res_e WPC_set_network_channel(net_channel_t channel)
{
    uint8_t att;
    att = channel;
    int res = csap_attribute_write_request(C_NETWORK_CHANNEL_ID,
                                           1,
                                           &att);
...
{
    int res = csap_attribute_read_request(C_MTU_ID,
                                          1,
                                          value_p);
    return convert_error_code(ATT_READ_ERROR_CODE_LUT, res);
}
...
{
    int res = csap_attribute_read_request(C_SCRATCHPAD_SEQUENCE_ID,
                                          1,
                                          value_p);
    return convert_error_code(ATT_READ_ERROR_CODE_LUT, res);
}
...
app_res_e WPC_get_mesh_API_version(uint16_t * value_p)
{
    app_res_e ret;
    uint8_t att[2];
    int res = csap_attribute_read_request(C_MESH_API_VER_ID,
                                          2,
                                          att);
...
{
    app_res_e ret;
    const uint8_t IDs[4] = {C_FIRMWARE_MAJOR_ID,
                            C_FIRMWARE_MINOR_ID,
                            C_FIRMWARE_MAINT_ID,
                            C_FIRMWARE_DEV_ID};
    ...
    for (int i=0; i < sizeof(IDs); i++) {
        uint8_t att[2];
        int res = csap_attribute_read_request(IDs[i],
                                              2,
                                              att);
...
{
    int res = csap_attribute_write_request(C_CIPHER_KEY_ID,
                                        16,
                                        key);
...
app_res_e WPC_is_cipher_key_set(bool * set_p)
{
    ...
    int res = csap_attribute_read_request(C_CIPHER_KEY_ID,
                                          16,
                                          key);
    if (res < 0)
    {
...
{
    int res = csap_attribute_write_request(C_AUTH_KEY_ID,
                                           16,
                                           key);
    return convert_error_code(ATT_WRITE_ERROR_CODE_LUT, res);
}
...
app_res_e WPC_is_authentication_key_set(bool * set_p)
{
    uint8_t key[16];
    // Try to read the key only to get the error code
    int res = csap_attribute_read_request(C_AUTH_KEY_ID,
                                          16,
                                          key);
                                 ...
                                 uint8_t * last_channel_p)
{
    app_res_e ret;
    uint8_t att[2];
    int res = csap_attribute_read_request(C_CHANNEL_LIM_ID,
                                          2,
                                          att);
...
app_res_e WPC_get_hw_magic(uint16_t * value_p)
{
    app_res_e ret;
    uint8_t att[2];
    int res = csap_attribute_read_request(C_HW_MAGIC,
                                          2,
                                          att);
...
app_res_e WPC_get_stack_profile(uint16_t * value_p)
{
    app_res_e ret;
    uint8_t att[2];
    int res = csap_attribute_read_request(C_STACK_PROFILE,
                                          2,
                                          att);
...
app_res_e WPC_get_channel_map(uint32_t * value_p)
{
    app_res_e ret;
    uint8_t att[4];
    int res = csap_attribute_read_request(C_CHANNEL_MAP,
                                4,
                                att);
...
app_res_e WPC_set_channel_map(uint32_t channel_map)
{
    ...
    int res = csap_attribute_write_request(C_CHANNEL_MAP,
                                 4,
                                 att);
...
static bool get_statck_status()
{
    ...
    while (WPC_get_stack_status(&status) != APP_RES_OK
            && max_attempt-- > 0)
    {
...
static app_res_e read_single_byte_msap(uint8_t att, uint8_t * pointer)
{
    int res;
    //app_res_e ret;
    res = msap_attribute_read_request(att,
                                      1,
                                      pointer);
...
app_res_e WPC_get_system_time(uint32_t * time_p)
{
    ...
    int res = msap_attribute_read_request(MSAP_SYSTEM_TIME,
                                          4,
                                          att);
                                     ...
                                     uint16_t * max_ac_p)
{
    app_res_e ret;
    uint8_t att[4];
    int res = msap_attribute_read_request(MSAP_ACCESS_CYCLE_RANGE,
                                          4,
                                          att);
                                      ...
                                      uint16_t * max_ac_l_p)
{
    app_res_e ret;
    uint8_t att[4];
    int res = msap_attribute_read_request(MSAP_ACCESS_CYCLE_LIMITS,
                                          4,
                                          att);
...
app_res_e WPC_get_current_access_cycle(uint16_t * cur_ac_p)
{
    app_res_e ret;
    uint8_t att[2];
    int res = msap_attribute_read_request(MSAP_CURRENT_ACCESS_CYCLE,
                                          2,
                                          att);
...
static const app_res_e SCRATCHPAD_STATUS_REMOTE_ERROR_CODE_LUT[] =
{
    APP_RES_OK, // 0
                                       ...
                                       uint16_t reboot_delay_s)
{
    ...
    int res = msap_scratchpad_remote_update(destination_address_le,
                                            seq,
                                            reboot_delay_s_le);
