...
int tsap_setTestMode_request (const uint32_t addr)
{
    ...
    memcpy(&request.payload.tsap_testModeSet_request_payload,
           &addr, sizeof(addr));
...
int tsap_setRadioChannel_request(const uint8_t radio_channel)
{
    ...
    memcpy(&request.payload.tsap_radioChannel_request_payload,
           &radio_channel, sizeof(radio_channel));
       res = WPC_Int_send_request(&request,  &confirm);
    LOGI("res = %d \n",res);
    if (res < 0)
        return res;
...
int tsap_setRadioPower_request (const int8_t dbm)
{
    ...
    memcpy(&request.payload.tsap_radioTXpower_request_payload,
           &dbm, sizeof(dbm));
       res = WPC_Int_send_request(&request,  &confirm);
    LOGI("res = %d \n",res);
    if (res < 0)
...
int tsap_sendRadioData(const tsap_radio_tx_data_req_pl_t * data,
                      uint32_t *sentBursts)
{
    ...
    request.payload_length = sizeof(data->hdr.len) +
                             sizeof(data->hdr.seq) +
                             sizeof(data->txCtrl.bursts) +
                             sizeof(data->txCtrl.ccaDuration) +
                             sizeof(data->txCtrl.txInterval) +
                             data->hdr.len;
    request.payload.tsap_radioSendData_request_payload.txCtrl.txInterval = data->txCtrl.txInterval;
    request.payload.tsap_radioSendData_request_payload.txCtrl.bursts = data->txCtrl.bursts;
    request.payload.tsap_radioSendData_request_payload.txCtrl.ccaDuration = data->txCtrl.ccaDuration;
    ...
    memcpy(&request.payload.tsap_radioSendData_request_payload.data,
           data->data, request.payload_length);
    res = WPC_Int_send_request(&request,  &confirm);
    LOGI("res = %d \n",res);
    if (res < 0)
        return res;
...
int tsap_sendRadioTestSignal(const tsap_radio_tx_test_signal_req_pl_t * data)
{
    ...
    request.payload_length = sizeof(data->signalType) +
                             sizeof(data->txCtrl.bursts) +
                             sizeof(data->txCtrl.ccaDuration) +
                             sizeof(data->txCtrl.txInterval);
    // If parameters are right FW enters into continuus TX loop and does not
    // send confirmation
    request.payload.tsap_radioSendTestSignal_request_payload.txCtrl.txInterval = data->txCtrl.txInterval;
    request.payload.tsap_radioSendTestSignal_request_payload.txCtrl.bursts = data->txCtrl.bursts;
    request.payload.tsap_radioSendTestSignal_request_payload.txCtrl.ccaDuration = data->txCtrl.ccaDuration;
...
int tsap_readRadioData(tsap_radio_rx_data_t * data)
{
    ...
    memcpy(data, &confirm.payload.tsap_radioDataRead_confirm_payload.rx,
           sizeof(tsap_radio_rx_data_t));
