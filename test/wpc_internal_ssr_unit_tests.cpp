...
int g_ssr_init_calls = 0;
int g_ssr_deinit_calls = 0;
int g_ssr_init_result = 0;
...
extern "C" int ssr_init(void)
{
    g_ssr_init_calls++;
    return g_ssr_init_result;
}
...
TEST(WpcInternalSsrUnitTest, DispatchCallbackRoutesSsrRegistrationIndications)
{
    ...
    WPC_Int_close();
    EXPECT_EQ(g_ssr_deinit_calls, 1);
    EXPECT_TRUE(g_platform_close_called);
    EXPECT_EQ(g_serial_close_calls, 1);
}

TEST(WpcInternalSsrUnitTest, InitializeFailsWhenSsrInitFails)
{
    g_dispatch_indication = nullptr;
    g_get_indication = nullptr;
    g_platform_close_called = false;
    ...
    EXPECT_EQ(WPC_Int_initialize("loopback", 125000), WPC_INT_GEN_ERROR);
    EXPECT_EQ(g_ssr_init_calls, 1);
    EXPECT_EQ(g_dsap_init_calls, 0);
    EXPECT_EQ(g_ssr_deinit_calls, 0);
    EXPECT_TRUE(g_platform_close_called);
    EXPECT_EQ(g_serial_close_calls, 1);
}
